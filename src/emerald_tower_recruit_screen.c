...
static u8 (*sEmeraldTowerSelect_CurrentOptionFunc)(void) = NULL;
...
static void Select_Task_HandleMenu(u8 taskId)
{
    ...
    case STATE_MENU_HANDLE_INPUT:
        if (JOY_NEW(A_BUTTON))
        {
            ...
            else if (retVal == SELECT_RECRUIT_MON)
            {
                gTasks[taskId].tState = 0;
                gTasks[taskId].func = Select_Task_Exit;
            }
...
static void CreateFrontierFactorySelectableMons(u8 firstMonId)
{
    ...
    while (i < SELECTABLE_MONS_COUNT)
    {
        ...
        species = ChooseRandomValidEmeraldTowerRecruitForm(baseSpecies);
        if (species == SPECIES_NONE)
            continue;

        for (j = 0; j < i; j++)
        {
            if (GET_BASE_SPECIES_ID(sFactorySelectScreen->mons[j + firstMonId].monId) == baseSpecies)
                break;
        }
        if (j != i)
            continue;
...
static bool32 IsEmeraldTowerRecruitSpeciesValid(enum Species species)
{
    ...
    if (speciesInfo->isRestrictedLegendary || speciesInfo->isSubLegendary || speciesInfo->isMythical)
        return TRUE;
...
static enum Species ChooseRandomValidEmeraldTowerRecruitForm(enum Species baseSpecies)
{
    const u16 *forms = gSpeciesInfo[baseSpecies].formSpeciesIdTable;
    u16 validForms[64];
    u8 count = 0;

    if (IsEmeraldTowerRecruitSpeciesValid(baseSpecies))
        validForms[count++] = baseSpecies;

    if (forms != NULL)
    {
        u8 i;

        for (i = 0; forms[i] != FORM_SPECIES_END && count < ARRAY_COUNT(validForms); i++)
        {
            enum Species form = forms[i];

            if (form != baseSpecies && IsEmeraldTowerRecruitSpeciesValid(form))
                validForms[count++] = form;
        }
    }
    ...
    return validForms[Random() % count];
}
...
static void Select_CopyMonsToPlayerParty(void)
{
    ...
    for (i = 0; i < SELECTABLE_MONS_COUNT; i++)
    {
        ...
        {
            gParties[B_TRAINER_PLAYER][partyCount] = sFactorySelectScreen->mons[i].monData;
            gSpecialVar_Result = sFactorySelectScreen->mons[i].monId;
            break;
        }
    }
...
static u8 Select_RunMenuOptionFunc(void)
{
    sEmeraldTowerSelect_CurrentOptionFunc = sSelect_MenuOptionFuncs[sFactorySelectScreen->menuCursorPos];
    return sEmeraldTowerSelect_CurrentOptionFunc();
}
...
void DoEmeraldTowerUnusedSwapScreen(void)
{
